...
class Target(object):
    def __init__(self, location, velocity, cycle, sigma_a, sigma, times, times2):
    ...
    def z_calculation(self):
        ...
        X_kk = np.zeros([self.times2, 4, self.times])
        X_kj = np.zeros([self.times2, 4, self.times])
        ...
        Z = np.zeros([self.times2, 2, self.times])
        for k in range(self.times2):
            np.random.seed(k + 10000)
            n1 = np.random.normal(0, self.sigma_a, self.times)
            w1 = np.random.normal(0, self.sigma, self.times)
            np.random.seed(k + 10086)
            ...
            w[0, :] = w1
            w[1, :] = w2
            Z[k, :, 0] = np.dot(H, X[:, 0]) + w[:, 0]
            for i in range(self.times - 1):
                # X matrix [x[0] x[1]...x[N-1]]
                X[:, i + 1] = np.dot(phi, X[:, i]) + np.dot(gamma, n[:, i])  # 4*N
                # Z matrix [z[0] z[1]...z[N-1]]
                Z[k, :, i + 1] = np.dot(H, X[:, i + 1]) + w[:, i + 1]  # 2*N
                # calculate X22
            for j in range(self.times - 1):
                X_kk[k, :, 0] = np.array([Z[k, 0, 1], (Z[k, 0, 1] - Z[k, 0, 0]) / self.cycle,
                                          Z[k, 1, 1], (Z[k, 1, 1] - Z[k, 1, 0]) / self.cycle]).T
                P_kk[0, :, :] = np.array([[p1, p2, 0, 0],
                                          [p2, p3, 0, 0],
                                          [0, 0, p1, p2],
                                          [0, 0, p2, p3]])
                # forecast
                X_kj[k, :, j] = np.dot(phi, X_kk[k, :, j])
                ...
                # filtering
                X_kk[k, :, j + 1] = X_kj[k, :, j] + np.dot(K[j, :, :], (Z[k, :, j] - np.dot(H, X_kj[k, :, j])))
    ...
    def plot(self, X, Z, X_kk, P_kk):
        times = np.arange(0, self.times-1)
        plt.subplot(1, 2, 1)
        plt.plot(X[0, :], Z[0, 1, :], color='gray', linestyle='--')
        plt.plot(X[0, :], X_kk[0, 2, :], color='orange')
        ...
        plt.subplot(1, 2, 2)
        lab = np.zeros(self.times-1)
        for i in range(self.times2) :
            lab += pow((X[0, 0:self.times-1]) - X_kk[i, 0, 1:self.times], 2)
        plt.plot(times, np.sqrt(lab/self.times2))
...
P1 = Target([-10000, 2000], [15, 0], 10, 0, 100, 100, 50)
